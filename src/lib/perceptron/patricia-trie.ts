// ============================================================================
// patricia-trie.ts - Patricia Trie for Vocabulary Indexing (Browser Version)
// ============================================================================
...
export class PatriciaTrie {
  ...
  private async hashNode(node: TrieNode): Promise<string> {
    try {
      // Check if crypto.subtle is available
      if (!crypto || !crypto.subtle) {
        console.warn('[PatriciaTrie] crypto.subtle not available, using fallback hash');
        // Fallback to simple string hash
        const nodeData = node.key + (node.isEndOfWord ? '1' : '0');
        const childData = Array.from(node.children.keys()).sort().join('|');
        return this.simpleHash(nodeData + childData);
      }
      ...
      for (const [key, child] of sortedChildren) {
        const childHash = await this.hashNode(child);
        childHashes.push(`${key}:${childHash}`);
      }

      const combinedData = nodeData + childHashes.join('|');
      const dataBytes = encoder.encode(combinedData);
      const hashBuffer = await crypto.subtle.digest('SHA-256', dataBytes);
      const hashArray = Array.from(new Uint8Array(hashBuffer));
      return hashArray.map(b => b.toString(16).padStart(2, '0')).join('');
    } catch (error) {
      console.error('[PatriciaTrie] Error computing hash:', error);
  ...
  getTopology(): TrieTopology {
    ...
    // Reserved for future metrics calculation
    // @ts-expect-error - Intentionally unused, reserved for future use
    const _avgTermLength = this.termCount > 0 ? stats.totalTermLength / this.termCount : 0;
  ...
  serialize(): any {
    ...
    return {
      ...
      trieHash: null, // Will be computed async
